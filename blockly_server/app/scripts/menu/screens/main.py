...
class MainMenuScreen(Screen):
    ...
    def update(self):
        low = self.is_low_batt()
        title = "FossBot Menu"
        line2 = "!! LOW BATTERY !!" if low else ""
    ...
    def handle_input(self, pressed_buttons):
        if pressed_buttons.get("bt1"):
            ...
            if selected == "Network":
                self.screen_manager.push_screen(NetworkScreen(self.robot, self.screen_manager))
            elif selected == "Battery":
                self.screen_manager.push_screen(BatteryScreen(self.robot, self.screen_manager, self.battery_monitor))
            elif selected == "Saved Networks":
                self.screen_manager.push_screen(SavedNetworksScreen(self.robot, self.screen_manager))
